...
def render_view(df_filtered):
    ...
    if view_mode == "Single table":
        ...
        for label in df_outcome_display["Label"].unique():
    ...
    with col1:
        weight_repeat = st.slider("Repeat call rate (7d) importance:", 0, 100, st.session_state.get("weight_repeat",33), key="weight_repeat")
    with col2:
        weight_churn = st.slider("Churn rate (30d) importance:", 0, 100, st.session_state.get("weight_churn",33), key="weight_churn")
    with col3:
        weight_cost = st.slider("Outcome cost importance:", 0, 100, st.session_state.get("weight_cost",34), key="weight_cost")
    with col4:
    ...
    with t_col1:
        low_threshold = st.slider("Low - medium boundary:", 0.0,1.0, st.session_state.get("low_threshold",0.33), step=0.01, key="low_threshold")
    with t_col2:
        med_threshold = st.slider("Medium - high boundary:",0.0,1.0, st.session_state.get("med_threshold",0.66), step=0.01, key="med_threshold")
    with t_col3:
    ...
    risk_df["risk_score"] = risk_df["repeat_score"]*w_repeat + risk_df["churn_score"]*w_churn + risk_df["cost_score"]*w_cost

    risk_df["risk_tier"] = pd.cut(risk_df["risk_score"], bins=[-0.01, low_threshold, med_threshold,1.01], labels=["Low","Medium","High"])
    risk_df["risk_pct"] = (risk_df["risk_score"]*100).round(1)

    tier_color_scale = alt.Scale(domain=["Low","Medium","High"], range=["#2ECC71","#FFB300","#E74C3C"])

    # view toggle
    view_toggle = st.radio("Choose view:", options=["All labels","Single label"], index=0, key="risk_view_toggle")
    ...
    if view_toggle=="Single label":
        ...
        selected_label = st.selectbox("Choose label:", options=single_labels, index=single_labels.index(default_label) if default_label in single_labels else 0, key="risk_label_select")
        label_df = risk_df[risk_df["Label"]==selected_label]
        height_single = min(300, 25*len(label_df["Outcome"].unique()))
        ...
        risk_chart_single = (
            ...
            .encode(
                ...
                y=alt.Y("Outcome:N", title=None, sort="-x", axis=alt.Axis(labelLimit=400, labelFontSize=12)),
                ...
                tooltip=[
                    ...
                    alt.Tooltip("Repeat Call Rate (7d):Q", title="Repeat Call Rate (7d)", format=".1%"),
        ...
        risk_chart_full = (
            ...
            .encode(
                x=alt.X("risk_pct:Q", title="Risk score (0–100%)", scale=alt.Scale(domain=[0,100])),
                y=alt.Y("Label:N", title=None, sort=alt.SortArray(label_order)),
                color=alt.Color("risk_tier:N", title="Risk tier", scale=tier_color_scale),
                tooltip=[
